...
import { PurchaseOrderTable } from "@/components/master-list/contract-matters-list";
...
export function PaymentClaimsList({
  ...
  const filteredPurchaseOrders = useMemo(
    () => filterPurchaseOrderRows(purchaseOrders, filters),
    [purchaseOrders, filters]
  );

  const activeFilters = countActiveMasterListFilters(filters);
  const rowCount = filteredPurchaseOrders.length;
  ...
  return (
    <Card>
      ...
      <CardContent className="p-0">
        ...
        <PurchaseOrderTable rows={filteredPurchaseOrders} showJoPaymentDescriptionColumn />
